...
function Game() {
    ...
    this.reset = () => {
        this.data = ["-", "-", "-", "-", "-", "-", "-", "-", "-"];
        // Might take out below when introducing random decider of who goes first
        this.currentPlayer = 1;
        this.computerPlaying = false;
    };
    this.buttons = () => {
    ...
    this.hard = () => {
        if (this.computerThinking > 0) {
            ...
            if (
                ...
                // If center is taken, go for a random corner
                console.log("2");
                ...
                let numArr = [1, 3, 5, 7];
                this.data[numArr[rnd4]] = compSide;
            } else if (edges.match(regP1) !== null && edges.match(regP1).length === 2 && !this.cPlayed) {
                // Two edges (not opposite - picked up by lookForWin function) have been played by opponent, fill in between them to stop them guaranteeing a win
